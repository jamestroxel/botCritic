...
// const cleanScore = Number(data.rt_score)
for (i=0; i<data.length; i++){
  data[i].rt_score = parseInt(data[i].rt_score, 10)
  data[i].release_date = parseInt(data[i].release_date, 10)
};
// avgYear = data.reduce(data[i].release_date)
...
slider.oninput = function() {
  ...
  let goodMovies = data.filter( function(entry){
    return entry.rt_score >= slider.value
  });
  var good = document.querySelector("#good")
  good.innerHTML = goodMovies.map(
    (item) => {
      return `<a href="https://www.rottentomatoes.com/search?search=${item.title}" target="_blank" class="result">${item.title}</a>`
    }
  ).join("");
  let badMovies = data.filter( function(entry){
    return entry.rt_score < slider.value
  });
  var bad = document.querySelector("#bad")
  bad.innerHTML = badMovies.map(
    (item) => {
      return `<a href="https://www.rottentomatoes.com/search?search=${item.title}" target="_blank" class="result">${item.title}</a>`
    }
  ).join("");
  ...
  // var goodTime = document.querySelector("#goodTime")
  var average = goodMovies.reduce((total, next) => total + next.release_date, 0) / goodMovies.length;
  goodTime.value = average;
  const badTime = document.getElementById("badTime");
  // var goodTime = document.querySelector("#goodTime")
  var average = badMovies.reduce((total, next) => total + next.release_date, 0) / badMovies.length;
  badTime.value = average;
  // goodMovies.map(
  //   () => {
  ...
  //////////////////////////////////////////////////
  // RELEASE DATE SKEW //
  // var goodSkew = document.querySelector("#goodDate")
  // releaseDate.innerHTML = goodmovies.map(
  //   (item) => {
  //   return `<li class="result">${item.releaseDate}</li>`
  // }
  // ).join("");
  // var badSkew = document.querySelector("#badDate")
  // releaseDate.innerHTML = badmovies.map(
  //   (item) => {
  //   return `<li class="result">${item.releaseDate}</li>`
  // }
  // ).join("");
...
// good.innerHTML = goodMovies.map(
//   (item) => {
//     return `<li class="result">${item.title}<br></li>`
//   }
// ).join("");
...
// bad.innerHTML = badMovies.map(
//   (item) => {
//     return `<li class="result">${item.title}</li>`
//   }
// ).join("");

// BUTTON WITH SLIDER

document.getElementById("botCritic").onclick = function() {
  let goodMovies = data.filter( function(entry){
    return entry.rt_score >= slider.value
  });
  var good = document.querySelector("#good")
...
// .enter().append('tr')
// .html(function(d) {
//   return '<th class="th-border">' + d.title +
//     '</th><td class="th-border">' + d.release_date + 
//     '</td><td class="th-border">' + d.rt_score + '</td>' 
// })
